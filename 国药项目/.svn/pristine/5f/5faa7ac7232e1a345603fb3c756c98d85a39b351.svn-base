...
{	
	...
	/// GY_SCTGBLL
	/// </summary>	
	public class GY_SCTGBLL
	{
	    private IGY_SCTGRepository service=new GY_SCTGRepository();

		/// <summary>
        /// 缓存key
        /// </summary>
        public string cacheKey ="GY_SCTGCache" ;
        ...
        public void auditSave(string keyValue, GY_SCTGEntity gY_SCTGEntity)
        {
            try
            {
                service.auditSave(keyValue, gY_SCTGEntity);
                CacheFactory.Cache().WriteCache(cacheKey, gY_SCTGEntity);
            }
            catch (Exception)
            {
                throw;
        ...
        /// <returns></returns>
        public IEnumerable<GY_SCTGEntity> GetPageListBySql(Pagination pagination, string queryJson)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public IEnumerable<GY_SCTGEntity> GetList( string queryJson)
        {
             var expression = LinqExtensions.True<GY_SCTGEntity>();
            var queryParam = queryJson.ToJObject();
            //查询条件
        ...
        /// <param name="queryJson">查询参数</param>
        /// <returns></returns>
        public IEnumerable<GY_SCTGEntity> GetPageList(Pagination pagination, string queryJson)
        {
            var expression = LinqExtensions.True<GY_SCTGEntity>();
            var queryParam = queryJson.ToJObject();
            ...
            {
                string keyord = queryParam["keyword"].ToString();
                expression = expression.And(t => t.activity.Contains(keyord));
            }
        ...
        public void SaveForm(string keyValue, GY_SCTGEntity gY_SCTGEntity, string ScoreList)
        {
            try
            {
                service.SaveForm(keyValue, gY_SCTGEntity, ScoreList);
                CacheFactory.Cache().WriteCache(cacheKey,gY_SCTGEntity);
            }
            catch (Exception)
            {
