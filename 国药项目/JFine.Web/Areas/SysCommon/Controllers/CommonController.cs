...
namespace JFine.Web.Areas.SysCommon.Controllers
{
    public class CommonController : JFControllerBase2
    {
        ...
        //base64压缩上传
        public JsonResult UploadBFIMobie()
        {
            Hashtable ht_result = new Hashtable();
            try
            {
                string modelName = Request["modelName"];//获取文件所属的模块
                string strbase64 = Request["uploadBF"].ToString();
                strbase64 = strbase64.Substring(strbase64.IndexOf(',') + 1).Trim('\0');
                ...
                using (MemoryStream ms = new MemoryStream(arr))
                {
                    ...
                    draw.Dispose();
                    bmp2.Save(saveFullPath, System.Drawing.Imaging.ImageFormat.Jpeg);
                    ms.Close();
                }
                bool uploaded = System.IO.File.Exists(saveFullPath);
                if (uploaded)
        ...
        public JsonResult Upload_bd_Excel()
        {
            ...
            {
                //判断编码
                DataRow[] arrayDR = dt.Select("NUMBER is null or NUMBER='' ");
                if (arrayDR != null && arrayDR.Length > 0)
                {
                    ht_result["status"] = "F";
                    ht_result["msg"] = "导入失败，含有编码为空的数据，请完善！";
                    return Json(ht_result);
                }
                //判断数量
                DataRow[] arrayDR1 = dt.Select("QUANTITY is null or QUANTITY = '' or QUANTITY = '0' ");
                if (arrayDR1 != null && arrayDR1.Length > 0)
                {
                    ht_result["status"] = "F";
                    ht_result["msg"] = "导入失败，数量应大于0，请完善！";
                    return Json(ht_result);
                }
                //判断编码
                DataRow[] arrayDR_unit = dt.Select("UNIT is null or UNIT='' ");
                if (arrayDR_unit != null && arrayDR_unit.Length > 0)
                {
                    ht_result["status"] = "F";
                    ht_result["msg"] = "导入失败，含有单位为空的数据，请完善！";
                    return Json(ht_result);
                }
                ...
                {
                    DataRow[] array_temp = dt.Select("NUMBER ='" + bdlist[i]["NUMBER"].ToString().Trim() + "'");
                    if (array_temp.Length > 1)
                    {
                        ht_result["status"] = "F";
                        ht_result["msg"] = "导入失败,标的（" + bdlist[i]["NUMBER"].ToString() + "）重复，请修正！";
                        return Json(ht_result);
                    }
                    try
                    {
