...
{	
	...
	/// GY_BLBLL
	/// </summary>	
	public class GY_BLBLL
	{
	    private IGY_BLRepository service=new GY_BLRepository();

		/// <summary>
        /// 缓存key
        /// </summary>
        public string cacheKey ="GY_BLCache" ;
        ...
        /// </summary>
        /// <returns></returns>
        public DataTable GetLists(Pagination pag,string queryjson)
        {
            ...
            {
                sqlwhere.Append(" AND PATIENTINFO.bfactoryname in ( SELECT name FROM SYS_ORGANIZE WHERE id = @CJid)");
                param.Add(DbParameters.CreateDbParameter("@CJid", queryparam["CJid"].ToString(), DbType.AnsiString));
            }
            if (!queryparam["manager"].IsEmpty())
            {
                sqlwhere.Append(" AND PATIENTINFO.hname in ( SELECT name FROM BHOSPITAL WHERE manager = @manager)");
            ...
            {
                sqlwhere.Append(" AND  PATIENTINFO.createuserid = @createuserid");
            ...
            {
                sqlwhere.Append(" AND  PATIENTINFO.createdate >= @startDate");
            ...
            {
                sqlwhere.Append(" AND  PATIENTINFO.createdate <= @endDate");
            ...
            {
                sqlwhere.Append(" AND  PATIENTINFO.hname = @searchYY");
            ...
            {
                sqlwhere.Append(" AND ID in (SELECT BindId FROM PATIENTINFOD WHERE pname LIKE @searchQX)");
                param.Add(DbParameters.CreateDbParameter("@searchQX", "%"+queryparam["searchQX"].ToString()+"%", DbType.AnsiString));
            }
            return service.GetLists(sqlwhere.ToString(), param, pag, "PATIENTINFO");
        }
        ...
        /// </summary>
        /// <returns></returns>
        public IEnumerable<GY_BLEntity> GetList()
        {
        ...
        /// </summary>
        /// <returns></returns>
        public IEnumerable<GY_BLEntity> GetListBySql( string sqlWhere)
        {
        ...
        /// <returns></returns>
        public IEnumerable<GY_BLEntity> GetPageListBySql(Pagination pagination, string queryJson)
        {
            var sqlWhere = new StringBuilder();
            var queryParam = queryJson.ToJObject();
        ...
        /// </summary>
        /// <returns></returns>
        public IEnumerable<GY_BLEntity> GetList( string queryJson)
        {
             var expression = LinqExtensions.True<GY_BLEntity>();
            var queryParam = queryJson.ToJObject();
            //查询条件
            if (!queryParam["Name"].IsEmpty())
            {
                string name = queryParam["Name"].ToString();
                expression = expression.And(t => t.pname.Contains(name));
            }
        ...
        /// <param name="queryJson">查询参数</param>
        /// <returns></returns>
        public IEnumerable<GY_BLEntity> GetPageList(Pagination pagination, string queryJson)
        {
			var expression = LinqExtensions.True<GY_BLEntity>();
            var queryParam = queryJson.ToJObject();
            ...
            if (!queryParam["keyword"].IsEmpty())
            {
                ...
                expression = expression.Or(t => t.CreateUserName.Contains(keyord));
                expression = expression.Or(t => t.operation.Contains(keyord));
            }
            //查询条件
        ...
        /// <returns></returns>
        public GY_BLEntity GetForm(string keyValue)
        {
        ...
        /// <returns></returns>
        public void SaveForm(string keyValue, GY_BLEntity gY_BLEntity, string ScoreList)
        {
            try
            {
                service.SaveForm(keyValue, gY_BLEntity, ScoreList);
                CacheFactory.Cache().WriteCache(cacheKey,gY_BLEntity);
            }
            catch (Exception)
            {
                throw;
        ...
        /// <param name="entity">实体</param>
        /// <returns></returns>
        public void SaveForm2(string keyValue, GY_BLEntity gY_BLEntity)
        {
            try
            {
                service.SaveForm2(keyValue, gY_BLEntity);
                CacheFactory.Cache().WriteCache(cacheKey, gY_BLEntity);
            }
            catch (Exception)
            {
