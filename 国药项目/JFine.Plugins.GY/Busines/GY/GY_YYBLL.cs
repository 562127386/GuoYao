...
{	
	...
	/// GY_YYBLL
	/// </summary>	
	public class GY_YYBLL
	{
	    private IGY_YYRepository service=new GY_YYRepository();

		/// <summary>
        /// 缓存key
        /// </summary>
        public string cacheKey ="GY_YYCache" ;
        ...
        /// <returns></returns>
        public IEnumerable<GY_YYEntity> GetPageListBySql(Pagination pagination, string queryJson)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public IEnumerable<GY_YYEntity> GetList( string queryJson)
        {
             var expression = LinqExtensions.True<GY_YYEntity>();
            var queryParam = queryJson.ToJObject();
            //查询条件
        ...
        /// <param name="queryJson">查询参数</param>
        /// <returns></returns>
        public IEnumerable<GY_YYEntity> GetPageList(Pagination pagination, string queryJson)
        {
			var expression = LinqExtensions.True<GY_YYEntity>();
            var queryParam = queryJson.ToJObject();
            ...
            {
                //string keyord = queryParam["keyword"].ToString();
                expression = expression.And(t => t.salesman.Contains(currentUser.UserName));
            }
        ...
        public void SaveForm(string keyValue, GY_YYEntity gY_YYEntity)
        {
            try
            {
                service.SaveForm(keyValue, gY_YYEntity);
                CacheFactory.Cache().WriteCache(cacheKey,gY_YYEntity);
            }
            catch (Exception)
            {
                throw;
